...
**Known issues:**
- **Content generation is gated behind backend deployment.** The Express backend is still local-only. On the live site the Dashboard now detects this via `BACKEND_AVAILABLE = !import.meta.env.PROD || !!import.meta.env.VITE_API_URL` ([client/src/pages/Dashboard.tsx](client/src/pages/Dashboard.tsx)) and renders in degraded mode: an amber "Heads up" banner above the two-column body, the "Generate content" pill relabeled to "Generation pending", the "Regenerate" pill hidden, calendar empty-day tiles showing "Coming soon" instead of "Generate this day", and the bio "Generate bio" / "Regenerate" buttons disabled. Existing posts, the saved Google bio, and business info still render normally. Once the backend is deployed and `VITE_API_URL` is set on Vercel, everything re-enables automatically with no code changes.
